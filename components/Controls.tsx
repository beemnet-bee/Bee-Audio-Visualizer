...
type ActiveTab = 'visualizer' | 'background' | 'lyrics' | 'output';
...
const Controls: React.FC<ControlsProps> = (props) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Settings Tabs */}
      <div className="p-1.5 bg-black/20 rounded-lg flex items-center justify-center space-x-1 md:space-x-2">
...
const Select: React.FC<{value: string, onChange: (e: React.ChangeEvent<HTMLSelectElement>) => void, children: React.ReactNode}> = ({value, onChange, children}) => (
    <select value={value} onChange={onChange} className="w-full bg-white/5 border border-white/10 text-white text-sm rounded-lg focus:ring-fuchsia-500 focus:border-fuchsia-500 p-2.5 pr-10 appearance-none bg-no-repeat bg-[position:right_0.7rem_center] bg-[size:1.2em_1.2em] bg-[url('data:image/svg+xml,%3csvg xmlns=%27http://www.w3.org/2000/svg%27 fill=%27none%27 viewBox=%270 0 20 20%27%3e%3cpath stroke=%27%23E040FB%27 stroke-linecap=%27round%27 stroke-linejoin=%27round%27 stroke-width=%271.5%27 d=%27M6 8l4 4 4-4%27/%3e%3c/svg%3e')]">
        {children}
    </select>
);
             ...
             <SettingGroup title="Editor">
                <div className="space-y-4 col-span-1 lg:col-span-2">
                 ...
                 <div className="space-y-2 col-span-1 lg:col-span-1">
                     ...
                     <div className="h-64 overflow-y-auto bg-black/20 rounded-lg p-2 space-y-2 border border-white/10">
...
const OutputSettings: React.FC<ControlsProps> = ({ resolution, onResolutionChange }) => {
    ...
    const ASPECT_RATIOS: {[key: string]: {w: number, h: number}} = {
        '16:9': { w: 16, h: 9 }, // Landscape
        '9:16': { w: 9, h: 16 }, // Portrait
        '1:1':  { w: 1, h: 1 },  // Square
        '4:3':  { w: 4, h: 3 },  // Classic
    };

    const RESOLUTION_PRESETS: {[key: string]: number} = {
        '720p': 720,
        '1080p': 1080,
        '1440p': 1440,
        '4K': 2160,
    ...
    const handleAspectRatioChange = (aspectRatioKey: string) => {
        ...
        if (ratio.w > ratio.h) { // Landscape or square/classic
            onResolutionChange({ width: Math.round(height * (ratio.w / ratio.h)), height: height });
        } else { // Portrait
            onResolutionChange({ width: height, height: Math.round(height * (ratio.h / ratio.w)) });
        }
    };
    ...
    const currentAspectRatioKey = Object.keys(ASPECT_RATIOS).find(key => {
        const ratio = ASPECT_RATIOS[key];
        return Math.abs((resolution.width / resolution.height) - (ratio.w / ratio.h)) < 0.01;
    }) || '16:9';
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="col-span-1 md:col-span-2 bg-black/20 p-3 rounded-lg text-center">
                <p className="text-sm text-gray-400">Final Output Size: <span className="font-bold text-fuchsia-300">{resolution.width}px &times; {resolution.height}px</span></p>
            </div>
        </div>
    );
};
